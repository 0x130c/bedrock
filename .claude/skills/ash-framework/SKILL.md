...
```sh
mix usage_rules.search_docs "search term" -p ash -p ash_admin -p ash_ai -p ash_archival -p ash_authentication -p ash_authentication_oauth2_server -p ash_authentication_phoenix -p ash_cloak -p ash_events -p ash_oban -p ash_paper_trail -p ash_phoenix -p ash_postgres -p ash_state_machine
```
...
- `mix ash_authentication_oauth2_server.install` - Scaffolds an OAuth 2.1 authorization server
